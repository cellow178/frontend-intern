...
interface FeedbackCategoryDetail {
  id: number
...
const category = ref<FeedbackCategoryDetail | null>(null)
...
const fetchDetail = async () => {
  ...
  try {
    const response = await api.get(`/feedbacks-categories/${route.params.id}`)
    category.value = response.data.data
  } catch (err) {
    console.error('Gagal mengambil detail kategori kritik & saran:', err)
...
<template>
  <div class="flex flex-col gap-4">
    <div class="bg-neutral rounded-2xl shadow-sm p-6 sm:p-8">
      <BackButton to="/dashboard/master-data?tab=feedback-categories" :sticky="false" />

      <div class="border-t border-text-alt/50 mt-2 mb-6"></div>

      <h1 class="text-xl font-bold text-text-neutral mb-4">Detail Kategori Kritik & Saran</h1>
      ...
      <dl v-else class="flex flex-col">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 py-4 border-b border-secondary">
          ...
          <dd class="sm:col-span-2 text-text-neutral">{{ category.category_name }}</dd>
        </div>

        <div class="border-t border-text-alt/10"></div>

        <div class="grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 py-4 border-b border-secondary">
          <dt class="text-text-neutral font-medium">Jumlah Feedback</dt>
          <dd class="sm:col-span-2 flex items-center gap-2">
            <span
              class="px-2.5 py-0.5 rounded-full text-sm font-semibold"
              :class="
                (category.feedbacks_count ?? 0) > 0
                  ? 'bg-primary/10 text-primary'
                  : 'bg-secondary/20 text-text-alt'
              "
            >
              {{ category.feedbacks_count ?? 0 }} Feedback
            </span>
          </dd>
        </div>
